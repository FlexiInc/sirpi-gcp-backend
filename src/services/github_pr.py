...
class GitHubPRService:
    ...
    async def create_infrastructure_pr(
        ...
        try:
            ...
            # 4. Commit all files
            for file in files:
                # Get filename from either 'path' or 'filename' key
                filename = file.get('path') or file.get('filename', '')
                
                if not filename:
                    logger.warning(f"Skipping file with no path/filename: {file.keys()}")
                    continue
                
                # Put Terraform files in terraform/ directory
                if filename.endswith('.tf'):
                    path = f"terraform/{filename}"
                else:
                    path = filename
                ...
                await self.github.create_or_update_file(
                    ...
                    message=f"Add {filename} (generated by Sirpi)",
                    branch=branch_name
                )
